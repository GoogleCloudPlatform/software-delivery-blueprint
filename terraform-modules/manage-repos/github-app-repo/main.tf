...
resource "null_resource" "set-repo" {
  ...
  provisioner "local-exec" {
    command = "${path.module}/prep-app-repo.sh ${var.org_name_to_clone_template_from} ${var.application_name} ${var.github_user} ${var.github_email} ${var.namespace[var.env[count.index]]} ${var.ksa[var.env[count.index]]} ${var.env[count.index]} ${count.index} ${var.region} ${var.secret_project_id} ${var.private_pool}"
  }
