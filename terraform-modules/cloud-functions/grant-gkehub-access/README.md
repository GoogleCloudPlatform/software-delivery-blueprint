...
1. This [Cloud Function][cloud-function] is used to provide connect gateway permissions to application CD service account.
2. The [Cloud Function][cloud-function] is triggered when an object arrives in the GCS bucket `gkehub-permission-fn-trg-bucket-${env}`.
3. The Application Factory while creating a new application writes the service account that needs this access to the GCS bucket `gkehub-permission-fn-trg-bucket-${env}` as an object. This invokes the Cloud Function that provides the service account the required access.
